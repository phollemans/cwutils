...
\subsection*{Version 4.2.0 -- July 2026}

\subsubsection*{\bigicon{box_software} Updates}

\begin{itemize}

\item The Java VM was updated to 17.0.19 for the latest bug and security fixes.

\item The user's guide was largely revised for the new CDAT functionality and 
better clarity and flow.

\item The color chooser for overlays in CDAT was expanded to include a larger 
and more useful color selection.

\item A new command line tool {\em cwagent} now provides help with using the
CoastWatch Utilities in a chat agent conversation.  The agent has access to 
the current documentation and can provide advice on tools, options, metadata,
and data processing workflows.  It can also be used to write automated data
processing scripts.

\item A new palette is available for displaying water classification 
data: Water-Class-23.

\item A link to the Unix-style command line tool manual pages is now added 
to the CoastWatch Utilities program group in the Windows Start Menu.  This 
helps Windows users with an easy reference rather than having to search
through the user's guide.

\item A new package is now available for Linux users with a .deb extension,
to be used with the dpkg package manager. 

\end{itemize}

\subsubsection*{\bigicon{bug_yellow} Bug fixes}
\begin{itemize}

\item NetCDF files read using NcML now correctly apply scale and 
offset metadata when reading variable values.

\item Polar projection data in cwrender and CDAT now more reliably render in 
the native orientation of the data file, rather than displaying rotated by 
180$^{\circ}$.
...
\subsection*{Version 4.1.5 -- December 2025}

\subsubsection*{\bigicon{box_software} Updates}

\begin{itemize}

\item The Java VM version for installation packages with bundled Java 
VMs is now 17.0.17 for the latest bug and security fixes.

\item The usage documentation was updated in various tools for clarity.

\item New palettes, overlay groups, and preferences have been added for the 
latest Coral Reef Watch data files.

\item CDAT and cwrender now use a higher speed expression parser, resulting
in 3x faster rendering in some cases of expression-based mask overlays.

\item Saved bitmask overlays used for testing cloud and quality flags
in CDAT now accept variable names with or without leading group names.
This makes it possible to use the same quality flag overlay with data files
that follow different variable group conventions.

\item The iQuam SST data reading capability in CDAT was updated to handle
the latest iQuam SST data files.

\item Various documentation was updated and corrected, including adding HTML
pages for the command-line tools.

\item CDAT has an added window size shortcut (Ctrl-4) to more easily display
wide aspect ratio global datasets.

\item CDAT startup resource errors are now more accurately reported.
...
\item Color palettes in CDAT and cwrender with few colors now map 
data values to colors more accurately.  Previously rounding errors resulted
in inaccurate mapping.

\item The CDAT palette chooser search field now correctly displays 
the clear button on Mac.

\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NEW VERSION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection*{Version 4.1.0 -- June 2025}
...
\item The Java VM version for installation packages with bundled Java 
VMs is now 17.0.14 for the latest bug and security fixes.

\item CDAT now updates the display when the user is using the color
enhancement text fields and hits the TAB key to move to the next field.

\item The cwanimate tool has been enhanced to:
\begin{itemize}
  \item Parse ISO style time values for the -{-}axis option
  \item Run in test mode using the new -{-}test option
  \item Access subsets of the gridded needed to render up to 20x faster
...
\item The hybrid CoastWatch / CF metadata NetCDF data format reader now
correctly passes on satellite and sensor metadata if specified, even if 
generic source metadata is also found.

\item The output 'source\_index' variable data from the cwcomposite tool 
using the -{-}savemap option now uses -1 for the specified fill value 
attribute instead of -32768 for consistency with actual fill values used in
the data array.

\item Cylindrical projection calculations from data in NetCDF files now 
correctly performs a latitude/longitude to row/column lookup in cases
where 1D geographic location vectors are specified in descending order rather
than ascending order.

\item Command-line calls written as metadata attributes into rendered images 
and output HDF and NetCDF data files now include quotes in parameters with
spaces in order to properly recreate command-line calls.

\item Output from the cwanimate tool now correctly labels the animation 
axis for step values greater than 1.

\item The CDAT file chooser now correctly highlights the selected file when
CDAT is opened by double clicking the file in a file manager, or when the
file is dragged onto the CDAT window.

\item The PDF files written by CDAT are now the same size as those written
from cwrender.  Previously they were 3-4 times larger due to a duplicate
image rendering issue.

\item The CDAT file chooser was crashing the application if the data preview
was still rendering when a new variable or file was selected.  This has
been fixed.

\item The color scale legend in CDAT now correctly adjusts its size when
the enhancement range is changed.  Previous versions truncated the color
scale.

\item Rectangular area surveys in CDAT now perform correctly even in cases
when the data image has been reoriented for display.

\item The histogram statistics panel for rectangular and polygon surveys in
CDAT now properly displays vertical bars in all cases, rather than diagonal
lines on some displays.

\item Polar stereographic projection data written in bottom-up rather
than top-down row ordering now displays correctly in CDAT.  Previously
it appeared mirrored.

\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NEW VERSION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection*{Version 4.0.1 -- December 2024}

\subsubsection*{\bigicon{box_software} Updates}

\begin{itemize}

\item The -{-}optimal option in cwcomposite now supports two new function
types for selecting the optimization variable data value: minabs and maxabs
for the minimum and maximum absolute value respectively.

\item ACSPO geostationary data format reading is updated to handle the metadata
conventions in version 2.90 files.

\item A new -{-}console option to cwgscript starts a BeanShell script console
for interactive testing of script snippets.

\item The new tool {\em cwanimate} creates animations of data from either local
files or datasets on ERDDAP, THREDDS, and OPeNDAP servers.
...
\item The hybrid CoastWatch / CF metadata NetCDF data format reader no 
longer incorrectly promotes integer data variables to double-precision 
floating-point data.
...
\subsection*{Version 4.0.0 -- May 2024}
...
\subsection*{Version 3.8.0 -- May 2023}
...
\subsection*{Version 3.7.1 -- July 2022}
...
\subsection*{Version 3.7.0 -- September 2021}
...
\subsection*{Version 3.6.1 -- March 2021}
...
\begin{itemize}
  ...
  \item GeoTIFFs created by cwrender and cwexport now have extra compression
...
\subsection*{Version 3.6.0 -- August 2020}
...
\subsection*{Version 3.5.1 -- November 2019}
...
\begin{itemize}
  ...
  \item Some Sentinel-1A data was being rendered in a mirrored orientation.
  This has been fixed.
...
\subsection*{Version 3.5.0 -- April 2019}
...
\subsection*{Version 3.4.1 -- October 2018}
...
\subsection*{Version 3.4.0 -- March 2018}
...
\subsection*{Version 3.3.2 -- October 2017}
...
\subsection*{Version 3.3.1 -- January 2016}
...
\subsection*{Version 3.3.0 -- October 2013}
...
\subsection*{Version 3.2.3 -- March 2009}
...
\subsection*{Version 3.2.2 -- November 2007}
...
\subsection*{Version 3.2.1 -- November 2006}
...
\subsection*{Version 3.2.0 -- October 2005}
...
\subsection*{Version 3.1.9 -- April 2005}
...
\subsection*{Version 3.1.8 -- November 2004}
...
\subsection*{Version 3.1.5 -- September 2003}
...
\subsection*{Version 3.1.6 -- December 2003}
...
\subsection*{Version 3.1.7 -- June 2004}
...
\subsection*{Version 3.1.4 -- May 2003}
...
\subsection*{Version 3.1.3 -- March 2003}
...
\subsection*{Version 3.1.2 -- December 2002}
...
\subsection*{Version 3.1.1 -- October 2002}
...
\subsection*{Version 3.1.0 -- July 2002}
...
\subsection*{Version 2.4 -- June 2001 (internal release)}
...
\subsection*{Version 2.3 -- October 1999}
...
\subsection*{Version 2.2 -- January 1999}
...
\subsection*{Version 2 -- September 1998 (first public release)}
...
\subsection*{Version 1 -- October 1997 (internal release)}
