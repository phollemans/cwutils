...
\noindent\textbf{Use when:} inspecting a data file, checking metadata, and doing quick quality checks.\par
\noindent\textbf{Typical tools:} \hyperlink{cwinfo}{cwinfo}, \hyperlink{cwstats}{cwstats} (and \hyperlink{hdatt}{hdatt} for low-level HDF attributes).\par
...
\noindent\textbf{Use when:} converting formats, exporting subsets, sampling values, masking, and building derived products.\par
\noindent\textbf{Typical tools:} \hyperlink{cwimport}{cwimport}, \hyperlink{cwexport}{cwexport}, \hyperlink{cwsample}{cwsample}, \hyperlink{cwmath}{cwmath}, \hyperlink{cwcomposite}{cwcomposite}, \hyperlink{cwtccorrect}{cwtccorrect}.\par
...
an Earth data reading software layer separating the tools from the physical input file format.
The \hyperlink{cwexport}{cwexport} tool complements cwimport -- it
converts data into various simple text or binary formats.  The
cwexport tool benefits from the reading layer as well, and as such can
...
median.  When processing large collections of files or running tools in batch
mode, it may be useful to adjust tool-wide runtime settings such as heap size 
or caching behavior. These advanced options apply to all command-line tools 
and are documented in \autoref{runtime} (\nameref*{runtime}).

The special purpose tool \hyperlink{cwtccorrect}{cwtccorrect} corrects 
top-of-atmosphere true color reflectance data for molecular (Rayleigh) scattering
and gaseous absorption (water vapor and ozone). The input file contains red/green/blue 
band data plus angles, and the output is a new set of corrected band reflectance data.
...
\noindent\textbf{Use when:} creating images and overlays for reports, or producing visualization outputs in batch.\par
\noindent\textbf{Typical tools:} \hyperlink{cwrender}{cwrender}, \hyperlink{cwgraphics}{cwgraphics}, \hyperlink{cwcoverage}{cwcoverage}, \hyperlink{cwanimate}{cwanimate}.\par
\noindent\textbf{Often combined with:} interactive exploration in CDAT to choose variables, ranges, palettes, and overlays before automating batch rendering.\par
...
surveys, and drawing annotations on top of the data image.  Interactive analysis 
in CDAT is often used to inform subsequent batch processing with the command 
line tools. In contrast, the command-line tools discussed in this section are designed
...
PNG, JPEG, GIF, GeoTIFF, and PDF.  To create MP4 movies of earth data, 
the \hyperlink{cwanimate}{cwanimate} tool 
can read one or more datasets (local or OPeNDAP) and render color mapped 
data frame by frame from the series of timesteps found in the input.
...
\noindent\textbf{Use when:} projecting data to a common map grid or comparing products on a shared master.\par
\noindent\textbf{Typical tools:} \hyperlink{cwmaster}{cwmaster}, \hyperlink{cwregister2}{cwregister2} (plus navigation/angle tools when needed).\par
