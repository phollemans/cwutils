...
/*
     FILE: AMSUAData.java
  PURPOSE: Reads AMSU-A instrument data.
...
/**
 * The <code>AMSUAData</code> class holds data from an AMSU-A
 * instrument on the NOAA KLMNN' series spacecrafts.
 *
 * @see AMSUA
 */
public class AMSUAData
  ...
  public int[] getCountData (
    ...
    // -----------------------
    if (countData == null) countData = new int[AMSUA.SAMPLES];

    // Get count data
    // --------------
    short[] sensorData = record.getSensorData();
    for (int i = 0; i < AMSUA.SAMPLES; i++)
      countData[i] = sensorData[i*AMSUA.CHANNELS + (channel-1)];
  ...
  public double[] getCalibratedData (
    ...
    // ----------------------------
    if (calData == null) calData = new double[AMSUA.SAMPLES];
