...
/*
     FILE: MHSData.java
  PURPOSE: Reads MHS instrument data.
...
/**
 * The <code>MHSData</code> class holds data from an MHS
...
public class MHSData
  ...
  public int[] getCountData (
    ...
    // -----------------------
    if (countData == null) countData = new int[MHS.SAMPLES];

    // Get count data
    // --------------
    short[] sensorData = record.getSensorData();
    for (int i = 0; i < MHS.SAMPLES; i++)
      countData[i] = sensorData[i*MHS.CHANNELS + (channel-1)];
  ...
  public double[] getCalibratedData (
    ...
    // ----------------------------
    if (calData == null) calData = new double[MHS.SAMPLES];
